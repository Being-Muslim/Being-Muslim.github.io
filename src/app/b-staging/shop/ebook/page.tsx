...
export default function ConceptBStagingShopEbook() {
  return (
    <>
      {/* ============ BREADCRUMB ============ */}
              ...
              <div style={css("display: flex; align-items: center; gap: 6px; margin-bottom: 16px")}>
                <div style={css("display: flex; gap: 2px")}>
                  {Array.from({ length: 5 }).map((_, idx) => (
                    <Star key={idx} style={idx < 4 ? css("width: 14px; height: 14px; fill: #d4a843; color: #d4a843") : css("width: 14px; height: 14px; color: #d8d2c8")} />
                  ))}
                </div>
                <span style={css("font-family: 'DM Sans', sans-serif; font-size: 13px; color: #8a7e70")}>4.7 (64 reviews)</span>
              </div>
