        ...
        <div className="mx-auto max-w-[1280px] px-6 lg:px-10">
          <div style={css("display: flex; justify-content: space-between; align-items: end; flex-wrap: wrap; gap: 16px; margin-bottom: 40px")}>
            <div>
              ...
              <h2 style={css("font-family: 'Hanken Grotesk', sans-serif; font-weight: 400; letter-spacing: -0.02em; font-size: clamp(30px, 4.5vw, 48px); line-height: 1.02; color: #2d3748; margin: 14px 0 0")}>Latest Resources</h2>
            </div>
            <Link href="/e/learn" className="bm-view-all bm-meta" style={css("text-decoration: none; display: inline-flex; align-items: center; gap: 8px; color: #8b2e36")}>
          ...
          <div style={css("max-width: 860px; margin: 0 auto")}>
            ...
            {faqs.map((faq, i) => (
              <div key={i} className="bm-faq-item">
                ...
                >
                  <span style={css("font-family: 'Hanken Grotesk', sans-serif; font-weight: 600; letter-spacing: -0.01em; font-size: 17px; color: #2d3748; line-height: 1.4")}>{faq.q}</span>
                  {openFaq === i ? (
                    <ChevronDown className="h-5 w-5 flex-shrink-0 transition-transform duration-200 rotate-180" strokeWidth={2} style={css("color: #8b2e36")} />
                  ) : (
                    <Plus className="h-5 w-5 flex-shrink-0" strokeWidth={2} style={css("color: #8b2e36")} />
                  )}
                </button>
                <div className={`bm-faq-answer ${openFaq === i ? "bm-faq-open" : ""}`}>
                  <p style={css("font-size: 16px; color: #6b7a8d; line-height: 1.65; margin: 0; padding: 0 26px 24px")}>{faq.a}</p>
                </div>
              </div>
            ))}
