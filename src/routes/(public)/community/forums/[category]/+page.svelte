...
<section class="bg-bg-dark py-16 sm:py-20">
	<div class="mx-auto max-w-[1280px] px-4 sm:px-6 lg:px-8">
		...
		<nav class="mb-6 flex items-center gap-2 text-sm text-text-cream/50">
			<a href="{base}/community" class="transition-colors hover:text-text-cream">Community</a>
			<ChevronRight class="h-3.5 w-3.5" />
			<a href="{base}/community/forums" class="transition-colors hover:text-text-cream">Forums</a>
			<ChevronRight class="h-3.5 w-3.5" />
	...
	{:else}
		<div class="divide-y divide-border overflow-hidden rounded-xl border border-border bg-surface">
			{#each sortedThreads as thread}
				<a
					href="{base}/community/forums/{data.category.slug}/{thread.slug}"
