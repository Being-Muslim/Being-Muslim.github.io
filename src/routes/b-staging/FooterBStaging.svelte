<footer style="background: #191918; padding: 64px 0 0;">
	<div class="mx-auto max-w-[1400px] px-6 lg:px-10">
		<div class="bm-grid-footer">
			<!-- Brand -->
			<div>
				<a href="/b-staging" style="display: flex; align-items: center; gap: 10px; text-decoration: none; margin-bottom: 16px;">
				...
				<p style="font-family: 'DM Sans', sans-serif; font-size: 13px; color: rgba(255,255,255,0.5); line-height: 1.6; max-width: 260px; margin-bottom: 20px;">
				...
				<div style="display: flex; gap: 10px;">
					{#each [
						...
						<a href="#" aria-label={social.label} class="bm-social-icon">
							<svg viewBox="0 0 24 24" style="width: 14px; height: 14px; fill: rgba(255,255,255,0.6);">
								<path d={social.icon} />
							</svg>
						</a>
					{/each}
			...
			<div>
				...
				{#each [
					{ label: 'Articles', href: '/b-staging/learn' },
					{ label: 'Courses', href: '/b-staging/learn' },
					{ label: 'Resources', href: '/b-staging/learn' }
				] as link}
			...
			<div>
				...
				{#each [
					{ label: 'The Book', href: '/b-staging/shop' },
					{ label: 'Boxed Set', href: '/b-staging/shop' },
					{ label: 'Prayer Cards', href: '/b-staging/shop' },
					{ label: 'eBook', href: '/b-staging/shop' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
			...
			<div>
				...
				{#each [
					{ label: 'Donate', href: '/b-staging/support' },
					{ label: 'Request Donation', href: '/b-staging/support' },
					{ label: 'Volunteer', href: '/b-staging/support' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
			...
			<div>
				...
				{#each [
					{ label: 'About Us', href: '/b-staging/contact' },
					{ label: 'Contact', href: '/b-staging/contact' },
					{ label: 'Convert', href: '/b-staging/convert' },
					{ label: 'Privacy Policy', href: '/b-staging' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
		...
		<div style="border-top: 1px solid rgba(255,255,255,0.08); padding: 20px 0; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 12px;">
			...
			<div style="display: flex; gap: 24px;">
				<a href="/b-staging" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Privacy</a>
				<a href="/b-staging" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Terms</a>
				<a href="/b-staging/contact" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Contact</a>
			</div>
		</div>
	</div>
</footer>
