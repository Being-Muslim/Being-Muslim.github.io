<footer style="background: #191918; padding: 64px 0 0;">
	<div class="mx-auto max-w-[1400px] px-6 lg:px-10">
		<div class="bm-grid-footer">
			<!-- Brand -->
			<div>
				<a href="/c-staging" style="display: flex; align-items: center; gap: 10px; text-decoration: none; margin-bottom: 16px;">
			...
			<div>
				...
				{#each [
					{ label: 'Articles', href: '/c-staging/learn' },
					{ label: 'Courses', href: '/c-staging/learn' },
					{ label: 'Resources', href: '/c-staging/learn' }
				] as link}
			...
			<div>
				...
				{#each [
					...
					{ label: 'Prayer Cards', href: '/c-staging/shop' },
					{ label: 'eBook', href: '/c-staging/shop' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
			...
			<div>
				...
				{#each [
					{ label: 'Donate', href: '/c-staging/support' },
					{ label: 'Request Donation', href: '/c-staging/support' },
					{ label: 'Volunteer', href: '/c-staging/support' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
			...
			<div>
				...
				{#each [
					{ label: 'About Us', href: '/c-staging/about' },
					{ label: 'Contact', href: '/c-staging/contact' },
					{ label: 'Convert', href: '/c-staging/convert' },
					{ label: 'Privacy Policy', href: '/c-staging' }
				] as link}
					<a href={link.href} class="bm-footer-link">{link.label}</a>
				{/each}
			</div>
		...
		<div style="border-top: 1px solid rgba(255,255,255,0.08); padding: 20px 0; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 12px;">
			...
			<div style="display: flex; gap: 24px;">
				<a href="/c-staging" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Privacy</a>
				<a href="/c-staging" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Terms</a>
				<a href="/c-staging/contact" class="bm-footer-link" style="font-size: 12px; color: rgba(255,255,255,0.3);">Contact</a>
			</div>
		</div>
	</div>
</footer>
