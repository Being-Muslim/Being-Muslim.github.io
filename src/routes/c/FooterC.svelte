<script lang="ts">
	...
	function socialEnter(e: MouseEvent) {
		(e.currentTarget as HTMLElement).style.background = 'rgba(212,168,83,0.15)';
	}
	function socialLeave(e: MouseEvent) {
		(e.currentTarget as HTMLElement).style.background = 'rgba(255,255,255,0.06)';
	}
</script>

<footer style="background: #0D0D0D; border-top: 1px solid rgba(255,255,255,0.06); padding: 64px 0 0;">
	<div class="bmc-container">
		<div class="bmc-grid-footer">
			<!-- Brand -->
			<div>
				<a href="/c" style="display: flex; align-items: center; gap: 12px; text-decoration: none; margin-bottom: 16px;">
					...
					<span style="font-family: 'Inter', sans-serif; font-size: 13px; font-weight: 600; letter-spacing: 0.12em; text-transform: uppercase; color: #F5F0E8;">BEING MUSLIM</span>
				</a>
				<p style="font-family: 'Inter', sans-serif; font-size: 13px; color: rgba(245,240,232,0.4); line-height: 1.7; max-width: 260px; margin-bottom: 24px;">
					Empowering converts and beginners on their Islamic journey through education, community, and support.
				</p>
				<!-- Social Icons -->
				<div style="display: flex; gap: 10px;">
					{#each socials as social}
						<a href="#" aria-label={social.label} class="bmc-social-icon"
							onmouseenter={socialEnter}
							onmouseleave={socialLeave}
						>
							<svg viewBox="0 0 24 24" style="width: 14px; height: 14px; fill: rgba(245,240,232,0.5);">
								<path d={social.icon} />
							</svg>
						</a>
					{/each}
			...
			<!-- Learn -->
			<div>
				<h4 style="font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 600; color: #D4A853; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 16px;">Learn</h4>
				{#each [
					{ label: 'Articles', href: '/c/learn' },
					{ label: 'Courses', href: '/c/learn' },
					{ label: 'Resources', href: '/c/learn' }
				] as link}
					<a href={link.href} class="bmc-footer-link">{link.label}</a>
				{/each}
			</div>

			<!-- Shop -->
			<div>
				<h4 style="font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 600; color: #D4A853; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 16px;">Products</h4>
				{#each [
			...
			<!-- Support -->
			<div>
				<h4 style="font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 600; color: #D4A853; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 16px;">Support</h4>
				{#each [
			...
			<!-- Company -->
			<div>
				<h4 style="font-family: 'Inter', sans-serif; font-size: 11px; font-weight: 600; color: #D4A853; text-transform: uppercase; letter-spacing: 0.1em; margin-bottom: 16px;">Company</h4>
				{#each [
		...
		<!-- Bottom bar -->
		<div style="border-top: 1px solid rgba(255,255,255,0.06); padding: 20px 0; display: flex; flex-wrap: wrap; justify-content: space-between; align-items: center; gap: 12px;">
			<p style="font-family: 'Inter', sans-serif; font-size: 12px; color: rgba(245,240,232,0.25);">&copy; 2026 The Being Muslim Project. All rights reserved.</p>
			<div style="display: flex; gap: 24px;">
				<a href="/c" class="bmc-footer-link" style="font-size: 12px; color: rgba(245,240,232,0.25);">Privacy</a>
				<a href="/c" class="bmc-footer-link" style="font-size: 12px; color: rgba(245,240,232,0.25);">Terms</a>
				<a href="/c/contact" class="bmc-footer-link" style="font-size: 12px; color: rgba(245,240,232,0.25);">Contact</a>
			</div>
		</div>
	</div>
</footer>
